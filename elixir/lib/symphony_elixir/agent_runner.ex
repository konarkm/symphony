defmodule SymphonyElixir.AgentRunner do
  ...
  defp linear_agent_prompt(opts) do
    ...
    - Use `linear_update_issue_state` for semantic state handoffs. Do not change state just because the AgentSession started or because you answered a conversational prompt.
    - Classify direct questions before changing state. If the issue or current prompt delegates the question itself as the task, answering is the work: move to `In Progress` when you begin and to `Human Review` or `Done` when the answer is delivered. If the issue already represents broader work and the prompt/comment is a question about that existing work, answer through Agent Activity and leave state unchanged unless you actually begin/change task execution or become blocked.
    - When you begin real implementation or task execution on an unstarted issue, move it to `In Progress`. For blocked work, emit an elicitation or error and move or leave the issue in `Blocked`. For ready handoff after real work, move it to `Human Review`. For genuinely complete non-review work, you may move it to `Done`.
